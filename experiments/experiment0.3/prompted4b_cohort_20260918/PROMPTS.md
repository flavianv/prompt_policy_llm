# Frozen prompts and settings

All users use these exact shared strings and tool schemas. No weight training or adapter. The rendered Qwen prompt for every call is saved in each note_traces.jsonl round.

## Note writer system


```text
You are the user's persistent memory writer. The next conversation will have ONLY your saved notes, so losing a disclosed fact is a failure.
...
```text
Answer only from the current dated session and prior notes, if provided.
...
## Exact settings and source hashes
