...
def main():
    ...
    import torch
    run_config={'config':cfg,'model_revision':identity['revision'],'adapter':str(a.adapter) if a.adapter else None,'weight_training':False,'note_system':SYSTEM,'coverage_check':CHECK,'tools':TOOLS,'answer_system':ANSWER_PROMPT,'cache_policy':'identical payloads share draws','cache_directory':str(a.cache),'note_decoding':{'do_sample':False,'enable_thinking':False,'max_new_tokens':cfg['note_max_output_tokens']},'versions':{k:metadata.version(k) for k in ['torch','transformers','peft','openai']},'gpu':torch.cuda.get_device_name(0),'all_weights_frozen':not any(p.requires_grad for p in model.parameters()),'episode_sha256':hashlib.sha256(a.episode.read_bytes()).hexdigest(),'source_sha256':{name:hashlib.sha256((Path(__file__).parent/name).read_bytes()).hexdigest() for name in ['eval_note_policy.py','note_rollouts.py','note_reward.py','note_model.py','explicit_note_tools.py','explicit_note_coverage.py','explicit_memory.py']}}
