...
NOTE_SYSTEM = """Maintain persistent notes about ONE user's current preferences.
Save explicit current preferences; ignore other people, hypotheticals, product
specifications and historical facts. Replace superseded preferences. Do not infer
demographic tastes. Use category names, at most 12 notes and 120 words total.
You are a note manager, not an answerer. Call exactly ONE tool per response.
Return only a single JSON object with keys tool and arguments; no prose or markdown.
Tools:
read_notes: arguments {}. Read all notes once before changes.
write_note: arguments {"name":"category","text":"concise current fact"}. Creates NEW name.
update_note: arguments {"name":"existing category","text":"replacement current fact"}.
no_op: arguments {}. Finish after needed changes, or immediately if none needed.
First call example: {"tool":"read_notes","arguments":{}}
A write example: {"tool":"write_note","arguments":{"name":"exercise","text":"User prefers swimming."}}
Finish example: {"tool":"no_op","arguments":{}}
Wait for the tool result before your next call. Do NOT emit multiple calls at once.
"""
...
def apply_calls(raw, notes, has_read):
    ...
    if not isinstance(body, dict) or set(body) != {'tool', 'arguments'}:
        raise ValueError('expected one tool/arguments object')
    calls = [body]
    ...
    for call in calls:
        ...
        if tool == 'read_notes':
            if args or has_read:
                raise ValueError('read_notes requires empty arguments and may run only once')
            read = True
            ...
            results.append({'tool': tool, 'ok': True, 'current_notes': dict(pending)})
        else:
        ...
        for round_index in range(6):
            ...
            messages.extend([{'role': 'assistant', 'content': generation['text']},
                             {'role': 'user', 'content': 'Tool results: ' + json.dumps(results) +
                              ('\nRead is complete. Return ONE write_note for a new category, update_note for an existing category, or no_op if all current facts are saved. Do not read again. Return exactly one JSON tool call.' if read else '\nCorrect the tool call and try again.')}])
            if done:
                break
...
def summarize(rows, note_rows):
    ...
    for r in rounds:
        if r['valid']:
            tools.update([strict_json(r['generation']['text'])['tool']])
...
def main():
    ...
    config = {'seeds': seeds, 'sessions': 10, 'profile_size': 5, 'update_rate': 0.35,
              ...
              'qwen': {'fixed_weights': True, 'thinking': False, 'do_sample': False, 'max_new_tokens': 384,
                       'max_tool_rounds': 6, 'precision': 'bfloat16', 'tool_protocol': 'strict JSON executed by local dispatcher; not native API tool_calls'},
