...
class StructuredRollout:
    ...
    def batch(self,observations,note_states,sample=False):
        ...
        for i,(obs,notes) in enumerate(zip(observations,note_states)):
            ...
            prompt=self.tokenizer.apply_chat_template([{'role':'system','content':SYSTEM},{'role':'user','content':json.dumps(payload,ensure_ascii=False)}],tokenize=False,add_generation_prompt=True,enable_thinking=False)
