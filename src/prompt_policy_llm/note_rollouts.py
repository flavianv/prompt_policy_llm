...
class NativeRollout:
    ...
    def update(self,obs,notes,sample=False):
        ...
        for i in range(self.config['note_max_rounds']):
            ...
            rounds.append({'round':i,'messages':deepcopy(messages),'generation':{'text':raw,'input_tokens':ids.shape[1],'output_tokens':len(tokens),'latency_s':time.perf_counter()-start,'hit_output_cap':len(tokens)>=cap},'valid':valid,'tool_result':tool_result,'notes_before':before,'notes_after':dict(notes)})
